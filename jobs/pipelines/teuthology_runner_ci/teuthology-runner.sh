...
# Script directory and log file (defaults if not set by Jenkins)
SCRIPT_DIR="${TEUTHOLOGY_DIR}"
LOG_FILE="${LOG_FILE:-${WORKSPACE:-$SCRIPT_DIR}/teuthology-runner.log}"
...
# --- Run a teuthology suite for a branch (suite from second arg; default smoke) ---
run_suite_for_branch() {
    ...
    local run_name=""
    local suite_output=""
    local tmp_err="${WORKSPACE}/tmp_err_${branch}_${suite}_$$.txt"
    local suite_output_file="${WORKSPACE}/suite_output_${branch}_${suite}_$$.txt"
    ...
    local tmp_out="${WORKSPACE}/tmp_out_${branch}_${suite}_$$.txt"
    ...
    log "Starting suite '$suite' for branch $branch with seed=$seed"
    ...
    if [[ "$actual_exit_code" -ne 0 ]]; then
        log "ERROR: Failed to schedule suite '$suite' for branch $branch (exit code: $actual_exit_code)"
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        local verify_output_file="${WORKSPACE}/verify_run_${branch}_${suite}_$$.txt"
        "${VIRTUALENV_PATH}/bin/python3" "${TEUTHOLOGY_PIPELINE_DIR:-${SCRIPT_DIR}}/verify_run.py" "$run_name" > "$verify_output_file" 2>&1
        local verify_exit=$?
    ...
    # Record run name for teuthology-rerun.sh (same path used by rerun script)
    ...
    local wait_output_file="${WORKSPACE}/teuthology_wait_${branch}_${suite}_$$.txt"
    local wait_exit=0
    ...
    log "Suite '$suite' for branch $branch completed successfully (teuthology-wait confirmed completion)"
    return 0
}

# --- Main: branch/suite from RUNNER_PARAMS (Jenkins) or optional args ---
main() {
    local branch="${BRANCH:-main}"
    local suite="smoke"
    if [[ "${RUNNER_PARAMS:-}" =~ --branch[[:space:]]+([^[:space:]]+) ]]; then
    ...
    if [[ -n "${2:-}" ]]; then
        suite="$2"
    elif [[ "${RUNNER_PARAMS:-}" =~ --suite[[:space:]]+([^[:space:]]+) ]]; then
        suite="${BASH_REMATCH[1]}"
    fi
    log "Log file: $LOG_FILE"
    cd "$SCRIPT_DIR" || exit 1
    if run_suite_for_branch "$branch" "$suite"; then
        log "Suite '$suite' for '$branch' branch completed"
        exit 0
    else
        log "Suite '$suite' for '$branch' branch had errors"
        exit 1
    fi
}
